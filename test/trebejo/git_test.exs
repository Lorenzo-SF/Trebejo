defmodule Trebejo.GitTest do
  ...
  describe "has_uncommitted_changes?/1" do
    ...
    test "returns false for a clean git repo" do
      ...
      refute Local.has_uncommitted_changes?(_work)
    end
    ...
    test "returns true for a dirty git repo" do
      ...
      assert Local.has_uncommitted_changes?(work)
    end
  end
  ...
  describe "get_current_branch/1" do
    ...
    test "returns the current branch name for a git repo" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:ok, "main"} = Local.get_current_branch(work)
    end
  end
  ...
  describe "ensure_clone/1" do
    ...
    test "returns repo_exists when path already exists" do
      ...
      assert {:repo_exists, ^repo} = Local.ensure_clone(repo, tmp)
    end
    ...
    test "clones a repo when path does not exist" do
      ...
      assert {:repo_cloned, ^clone_repo} = Local.ensure_clone(clone_repo, tmp)
    ...
    test "processes a list of repos" do
      ...
      results = Local.ensure_clone([repo1, repo2], tmp)
  ...
  describe "sync/1" do
    ...
    test "syncs a single repo map" do
      ...
      assert {:ok, %{path: ^work}} = Local.sync(repo_map)
    end
  ...
  describe "clone/1" do
    ...
    test "clones a repository from url to path" do
      ...
      clone_path = Path.join(tmp, "cloned")
      clone_repo = %{url: repo.url, path: clone_path}
      assert {:ok, ^clone_repo} = Local.clone(clone_repo)
      assert File.dir?(clone_path)
  ...
  describe "clone_repository/2" do
    ...
    test "clones a url to a target path" do
      ...
      on_exit(fn -> File.rm_rf!(tmp) end)
      clone_path = Path.join(tmp, "cloned")
      assert {:ok, _} = Local.clone_repository(repo.url, clone_path)
      assert File.dir?(clone_path)
    end
  end
  ...
  describe "checkout/1" do
    ...
    test "checks out the main branch" do
      ...
      repo_map = %{path: work, main_branch: "main"}
      assert {:ok, ^repo_map} = Local.checkout(repo_map)
      assert {:ok, "main"} = Local.get_current_branch(work)
    end
  end
  ...
  describe "commit/2" do
    ...
    test "stages and commits changes" do
      ...
      Local.add(work, :all)
      assert {:ok, ^repo_map} = Local.commit(repo_map, "test commit")
    end
  end
  ...
  describe "stage_and_commit/2" do
    ...
    test "adds and commits changes in one step" do
      ...
      assert {:ok, ^repo_map} = Local.stage_and_commit(repo_map, "stage and commit")
    end
  end
  ...
  describe "conflict_files/1" do
    ...
    test "returns empty list for a clean repo" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:ok, []} = Local.conflict_files(work)
    end
  ...
  describe "merge_abort/1" do
    ...
    test "returns error when no merge is in progress" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:error, _} = Local.merge_abort(work)
    end
  end
  ...
  describe "mark_resolved/2" do
    ...
    test "stages a file and returns :ok" do
      ...
      assert :ok = Local.mark_resolved(work, "conflicted.txt")
    end
    ...
    test "returns error for non-existent file" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:error, _} = Local.mark_resolved(work, "nonexistent.txt")
    end
  end
  ...
  describe "has_stash?/1" do
    ...
    test "returns true after stashing" do
      ...
      {:ok, _} = Local.stash_push(work)
      assert Local.has_stash?(work)
    end
  end
  ...
  describe "stash_if_needed/1" do
    ...
    test "returns {:ok, :clean} when no changes" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:ok, :clean} = Local.stash_if_needed(work)
    end
    ...
    test "returns {:ok, {:stashed, branch}} when there are changes" do
      ...
      assert {:ok, {:stashed, "main"}} = Local.stash_if_needed(work)
    end
  end
  ...
  describe "add/2" do
    ...
    test "single file returns ok" do
      ...
      assert {:ok, _} = Local.add(work, "single.txt")
    end
    ...
    test "list of files returns :ok" do
      ...
      assert :ok = Local.add(work, ["a.txt", "b.txt"])
    end
  end
  ...
  describe "fetch/1" do
    ...
    test "fetches from origin" do
      ...
      on_exit(fn -> File.rm_rf!(tmp) end)
      repo = %{path: work}
      assert {:ok, ^repo} = Local.fetch(repo)
    end
  end
  ...
  describe "pull/1" do
    ...
    test "pulls from origin" do
      ...
      assert {:ok, ^repo} = Local.pull(repo)
    end
  end
  ...
  describe "fetch_origin/1" do
    ...
    test "fetches and returns output" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:ok, _} = Local.fetch_origin(work)
    end
  end
  ...
  describe "pull_origin/2" do
    ...
    test "pulls default branch" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:ok, _} = Local.pull_origin(work)
    end
  end
  ...
  describe "stash_push/2" do
    ...
    test "stashes uncommitted changes" do
      ...
      assert {:ok, _} = Local.stash_push(work)
    end
    ...
    test "stashes with custom message" do
      ...
      {:ok, output} = Local.stash_push(work, message: "my stash", include_untracked: true)
  ...
  describe "blame/2" do
    ...
    test "returns blame info for a file" do
      {tmp, work, _repo} = setup_git_repo()
      on_exit(fn -> File.rm_rf!(tmp) end)
      assert {:ok, _} = Local.blame(work, "README.md")
    end
  end
  ...
  describe "diff/1" do
    ...
    test "returns diff of changes" do
      ...
      assert {:ok, diff} = Local.diff(work)
    ...
    test "returns staged diff" do
      ...
      Local.add(work, :all)
      assert {:ok, diff} = Local.diff(work, staged: true)
  ...
  describe "churn/2" do
    ...
    test "returns churn metrics for repo" do
      ...
      Local.add(work, :all)
      Local.commit(repo, "add foo")

      File.write!(Path.join(work, "README.md"), "## Updated Readme\n")
      Local.add(work, :all)
      Local.commit(repo, "update readme")

      File.write!(Path.join(work, "lib/foo.ex"), "defmodule Foo do\n  def bar, do: :ok\nend")
      Local.add(work, :all)
      Local.commit(repo, "update foo")

      assert {:ok, results} = Local.churn(work, period: "1.year", top: 10)
    ...
    test "returns empty list for new repo with no history" do
      ...
      case Local.churn(work, period: "1.day", top: 10) do
        {:ok, _} -> :ok
        {:error, _} -> :ok
      end
    end
  ...
  describe "set_user_info/2" do
    ...
    test "sets user info globally" do
      # assert :ok = Local.set_user_info("Test User", "test@example.com")
    end
  end
