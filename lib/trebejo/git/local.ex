defmodule Trebejo.Git.Local do
  ...
  def churn(repo_path \\ ".", opts \\ []) do
    ...
    git_opts = build_git_opts(repo_path, Keyword.get(opts, :timeout))

    case run_git(args, git_opts) do
      {:ok, %{exit_code: 0, stdout: out}} ->
  ...
  defp build_churn_args(opts) do
    ...
    format = if include_authors, do: "--format=COMMIT:%an", else: "--pretty=format:"
    limit = if max_commits, do: ["--max-count=#{max_commits}"], else: ["--since=#{period}"]
  ...
  defp build_git_opts(repo_path, nil), do: [cd: repo_path]
  defp build_git_opts(repo_path, timeout), do: [cd: repo_path, timeout: timeout]

  defp contains_no_commits?(err) do
    String.contains?(err, "no commits") or String.contains?(err, "no tiene ningún commit")
  end

  defp parse_churn_output(output, include_authors?) do
